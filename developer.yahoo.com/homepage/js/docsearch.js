...
YUI.YDN.docs.searchModule = (function()  {
    ...
    YUI().use('io', 'substitute', 'gallery-yql', 'event-key', function(Y)  {
        ...
        var showSearchResults = function(searchResults)  {
            ...
            var resultsOutput = Y.one('#ydn-search-module-results'),
            // this is a template that's used to generate the LIs 
            // for each search result
            liTemplate = "<li><a href="https://developer.yahoo.com/homepage/js/\&quot;{url}\&quot;">{title}</a></li>",
	...
	if (searchResults.count == 1) {
		dispcnt++;
		tu = {
			"url": searchResults.results.BATCHSEARCHRESPONSE.RESULTSET_WEB.RESULT.URL,
			"title": searchResults.results.BATCHSEARCHRESPONSE.RESULTSET_WEB.RESULT["TITLE.HTML.HL"]
		};
		markup += Y.Lang.substitute(liTemplate, tu);
		tu = {};
	} else {
                for (i = 0; i < searchResults.count; i+=1) {
			if(urllevel == 0 || (urllevel == 1 && searchResults.results.BATCHSEARCHRESPONSE.RESULTSET_WEB.RESULT[i].URL.indexOf(urlbase) !== -1))
			{
			    dispcnt++;
			    if(dispcnt > 10)
				break;
        ...
        fetchData = function(e, truesearch)  {
            ...
            input = Y.one('#search-input'),

            // make the YQL call. Use the path var from above and what the user has typed 
            // into the input box
            yqlQuery = new Y.yql('select * from json where url=\'https://developer.yahoo.com/homepage/docsearch.html?query=' + encodeURIComponent(input.get("value")) + '&path=' + encodeURIComponent(path) + '\'');
